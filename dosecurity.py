...
NVIDIA_API_KEY = "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"
...
WARNING_LEVELS = {
    ...
    3: {"action": "timeout", "duration": 2, "message": "🔇 Cảnh báo lần 3: Bạn bị mute 2 phút!"},
    4: {"action": "timeout", "duration": 5, "message": "🔇 Cảnh báo lần 4: Bạn bị mute 5 phút!"},
    5: {"action": "timeout", "duration": 10, "message": "🔇 Cảnh báo lần 5: Bạn bị mute 10 phút!"},
    6: {"action": "kick", "duration": 0, "message": "👢 Cảnh báo lần 6: Bạn bị kick khỏi server!"},
    7: {"action": "ban", "duration": 0, "message": "🔨 Cảnh báo lần 7: Bạn bị ban vĩnh viễn!"}
}
...
async def warn_user(ctx, member: discord.Member, *, reason: str = "Không có lý do"):
    ...
    if warning_count in WARNING_LEVELS:
        ...
        embed.add_field(name="🔢 Lần cảnh báo", value=f"{warning_count}/7", inline=True)
...
async def check_warnings(ctx, member: discord.Member = None):
    ...
    embed.add_field(name="👤 Thành viên", value=member.mention, inline=True)
    embed.add_field(name="⚠️ Số cảnh báo", value=f"{warning_count}/7", inline=True)
    
    if warning_count > 0:
        next_level = min(warning_count + 1, 7)
...
async def user_info(ctx, member: discord.Member = None):
    ...
    embed.add_field(name="⚠️ Cảnh báo", value=f"{data_manager.get_warnings(ctx.guild.id, member.id)}/7", inline=True)
