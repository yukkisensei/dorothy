...
class DataManager:
    ...
    def load_data(self):
        if os.path.exists(self.filename):
            with open(self.filename, 'r', encoding='utf-8') as f:
                return json.load(f)
        return {"warnings": {}, "kicks": {}, "bans": {}, "mutes": {}, "prefixes": {}}
    ...
    def get_prefix(self, guild_id: str) -> str:
        """Get custom prefix for a server (default: -)"""
        guild_id = str(guild_id)
...
def get_prefix(bot, message):
    """Get prefix for the current guild"""
    if message.guild:
...
async def help_command(ctx):
    ...
    embed.add_field(
        name="⚙️ **Cấu hình & Tiện ích**",
        value=(
            "`-setprefix <prefix>` - Đổi prefix cho server\n"
            "`-say <nội dung>` - Bot nói thay bạn\n"
            "`-say -r <id> <nội dung>` - Bot reply tin nhắn"
        ),
        inline=False
    )
    
    current_prefix = data_manager.get_prefix(ctx.guild.id) if ctx.guild else PREFIX
    embed.set_footer(text=f"{BOT_NAME} v2.0 | Prefix: {current_prefix}")
    await ctx.send(embed=embed)
...
async def set_prefix_command(ctx, new_prefix: str = None):
    """Đặt prefix tùy chỉnh cho server (mặc định: -)"""
...
async def say_command(ctx, *, content: str = None):
    """Bot nói thay bạn. Dùng -say -r <message_id> <content> để reply tin nhắn
    Usage: 
    -say <content> - Bot nói
    -say -r <message_id> <content> - Bot reply tin nhắn
    """
    # Xóa tin nhắn của người dùng ngay lập tức
    try:
    ...
    if content.startswith('-r '):
        ...
        try:
            ...
            temp_msg = await ctx.send(f"❌ Lỗi: {str(e)}")
            await asyncio.sleep(3)
            try:
                await temp_msg.delete()
            except:
